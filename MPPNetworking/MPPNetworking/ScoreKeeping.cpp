...
void ScoreKeeping::sendData(TCPsocket server, bool& quit_)
{
    //char buffer[1024];
    ...
    //        //    std::string message = std::string(response) + std::string(userBuffer);
    //        //    SDLNet_TCP_Send(client, message.c_str(), strlen(message.c_str()) + 1);
    ...
    //        {
    //            //TCP Send
    //            std::string score_data = serializeScore();
    //            SDLNet_TCP_Send(client, score_data.c_str(), score_data.size());
    //        }
    //        else
    //            break;
    //    }
